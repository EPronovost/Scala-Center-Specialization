...
object TimeUsage {
  ...
  def timeUsageByLifePeriod(): Unit = {
    ...
    val typedDS = timeUsageSummaryTyped(summaryDf).cache()
    typedDS.show()
    val finalDf3 = timeUsageGroupedTyped(typedDS)
    finalDf3.show()
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val (primaryNeeds, rest1) = columnNames.partition(s => primaryNeedsPrefixes.exists(s.startsWith(_)))
    val (workNeeds, rest2) = rest1.partition(s => workPrefixes.exists(s.startsWith(_)))
    val otherNeeds = rest2.filter(s => otherPrefixes.exists(s.startsWith(_)))
    
    (primaryNeeds.map(col(_)), workNeeds.map(col(_)), otherNeeds.map(col(_)))
  }
  ...
  def timeUsageSummary(
    ...
    val workingStatusProjection: Column =
      when((df("telfs").as[Double] >= 1) && (df("telfs").as[Double] < 3), "working")
        .otherwise("not working")
        .as("working")
    val sexProjection: Column = when(df("tesex").as[Double] === 1, "male").otherwise("female").as("sex")
    val ageProjection: Column =
      when((df("teage").as[Double] >= 15) && (df("teage").as[Double] <= 22), "young")
        .when((df("teage").as[Double] >= 23) && (df("teage").as[Double] <= 55), "active")
  ...
  def timeUsageGroupedSqlQuery(viewName: String): String =
    s"""SELECT working, sex, age,
          avg(primaryNeeds) AS primaryNeeds, avg(work) AS work, avg(other) AS other
        FROM $viewName
  ...
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] =
    timeUsageSummaryDf.map(r => TimeUsageRow(r.getAs[String]("working"),
      r.getAs[String]("sex"), r.getAs[String]("age"), r.getAs[Double]("primaryNeeds"),
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed.groupByKey(r => (r.working, r.sex, r.age))
      ...
      .map( {case (key, primaryNeeds, work, other) => TimeUsageRow(key._1, key._2, key._3, primaryNeeds, work, other)})
  }
}
