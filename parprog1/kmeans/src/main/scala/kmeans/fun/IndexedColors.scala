...
class IndexedColorFilter(initialImage: Img,
  ...
  private def imageToPoints(img: Img): GenSeq[Point] =
    for (x <- ParRange(0, img.width, 1, false);
         y <- ParRange(0, img.height, 1, false))
      yield {
      val rgba = img(x, y)
    ...
    val initialPoints: GenSeq[Point] =
      initStrategy match {
        ...
        case UniformSampling =>
          val sep: Int = 32
          (for (r <- ParRange(0, 255, sep, false); g <- ParRange(0, 255, sep, false); b <- ParRange(0, 255, sep, false)) yield {
        ...
        case UniformChoice =>
          ...
          for (r <- ParRange(0, 256, d, false); g <- ParRange(0, 256, d, false); b <- ParRange(0, 256, d, false)) yield
